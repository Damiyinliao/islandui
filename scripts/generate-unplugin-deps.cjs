...
config.components.forEach((element) => {
  // item.packages.forEach((element) => {
    styleMap.set(element.name, {
      name: element.name
    })
    if (target === 'islandui-taro' && element.taro === false) return // 排除无 Taro 版组件
    // gen entry
    if (element.exclude !== true) {
      let outputMjs = ''
      if (element.funcCall === true && target == 'islandui') {
...
tasks.push(
  fs.outputFile(path.resolve(__dirname, `../dist/islandui.es.js`), outputFileEntry, 'utf8', () => {
    // console.log('')
  })
)
