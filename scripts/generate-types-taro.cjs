...
const basePath = path.join(toDir, 'components')
...
declare module 'vue' {
    interface GlobalComponents {
        Island${componentName}: typeof _default;
    }
}`
        if (setup) {
          let changeContent = content.replace(
            'export default _default;',
            `declare const _island_default: WithInstall<typeof _default>;\nexport default _island_default;\n${remain}`
          )
